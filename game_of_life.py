...
class GameOfLife:
    ...
    def set_board(self, width, height):
        ...
        self.neighbours_position = np.array(
    ...
    def run(self, nb_max_iterations=1):
        for _ in range(nb_max_iterations):
            ...
            self.board = np.where(
                self.board,
                self.rule_alive_neighbours[neighbours_count],
                self.rule_dead_neighbours[neighbours_count],
            )
    ...
    def get_neighbours(self):
        return np.array(
            [
                np.sum(self.board[(idx + self.neighbours_position) % (self.width * self.height)])
